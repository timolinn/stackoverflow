...
export const renderEmail = (req: Request, res: Response, next: NextFunction) => {
  const html = sendgrid.templetify(req.params.template, { name: "Tim" });
  if (!html || html === "") {
    return res.json(error("template not found", StatusCodes.NOT_FOUND,
      [{ message: "template not found", reason: ErrorNames.ResourceNotFound }]));
  }
  return res.send(html);
};

export const signJWT = (user: UserInterface, options: jwt.SignOptions) => jwt.sign({
  userId: user.id,
  sub: user.id,
  role: user.role,
}, config.jwt.secret, options);

export const signRefreshJWT = (user: UserInterface, options: jwt.SignOptions) => jwt.sign({
  userId: user.id,
  sub: user.id,
  role: user.role,
